...
class userValidation extends command {
  ...
  constructor(bot){
    this.bot = this.bot;
    this.tokens = {};
    this.validationMessage = ( config.message || "voici le token pour Discord : \n \n !UB token {token}")
    this._register();
  }

  _register(){
    this.bot.commands["pseudo"] = this.bot.commands["identifie-moi-comme"] =  this._askForValidation
    this.bot.commands["token"] = this._checkAndValidateUser
  }
