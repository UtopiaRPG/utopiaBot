    ...
    if (this.tokens[name] != null) {
      ...
      this.browser.loadProfile(user.profile).then(body => {
        if (body.indexOf(this.tokens[name].token) != -1) {
          ...
          console.log(this.bot.config.rolesID.verify)
          Promise.all([
            userGuild.setNickname(pseudo),
            userGuild.addRole(message.guild.roles.find(role => role.id == this.bot.config.rolesID.verify)) //message.guild.roles.get("name",config.roles.verify).id);
          ]).then(() => {
